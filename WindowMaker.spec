...
Summary(fr):	Gestionnaire de fenÍtres avec le look NeXT
Summary(pl):	Menedøer okien w stylu NeXT
Name:		WindowMaker
Version:	0.70.0
Release:	1
...
%description -l fr
Window Maker est un gestionnaire de fenÍtres pour X11 qui cherche ‡
reproduire l'allure et l'ergonomie ("look & feel") de l'interface
graphique NeXTSTEP(tm) (aka OpenStep). Il est relativement rapide,
ÈvoluÈ, et facile ‡ configurer et ‡ utiliser. Window Maker fait
officiellement partie du projet GNU, ce qui signifie que Window Maker
peut coopÈrer avec d'autres projets GNU, comme par exemple GNOME.

Window Maker permet de changer de thËmes facilement, de placer ses
applications favorites soit sur un "dock" similaire au programme Wharf
de AfterStep, soit sur un dock intÈgrÈ ‡ l'espace de travail, appelÈ
"clip" (trombone), et qui permet d'Ètendre les possibilitÈs du dock
principal.

Vous devriez installer ce package si votre gestionnaire de fenÍtres
est Window Maker, ou si vous voulez l'essayer. Si vous installez le
package Window Maker, vous voudrez peut-Ítre installer aussi le
package AfterStep-APPS, qui contient des "applets" (petites
applications) qui fonctionnent ‡ la fois dans les gestionnaires de
fenÍtres AfterStep et Window Maker.

%description -l pl
WindowMaker jest menedøerem okien przypominaj±cym wygl±dem i wygod±
obs≥ugi interfejs systemu NeXTSTEP(tm). Jest szybki, stosunkowo ma≥y,
o duøych moøliwo∂ciach i ≥atwy w konfiguracji. Konfiguruje siÍ go
myszk±, za pomoc± programu WPrefs wchodz±cego w sk≥ad tego pakietu.

%package libs
Summary:	WindowMaker shared libraries
Summary(pl):	Biblioteki wspÛ≥dzielone WindowMakera
Group:		Libraries
Group(de):	Libraries
...
Ten pakiet zawiera biblioteki wspÛ≥dzielone niezbÍdne do pracy
menedøera okien WindowMaker.

%package devel
Summary:	WindowMaker libraries - development part
Summary(fr):	Librairies de WindowMaker
Summary(pl):	Biblioteki WindowMakera - czÍ∂Ê dla programistÛw
...
Ten pakiet zawiera pliki nag≥Ûwkowe i biblioteki niezbÍdne do
tworzenia aplikacji wykorzystuj±cych moøliwo∂ci menedøera okien
WindowMaker.
...
Ten pakiet zawiera statyczne biblioteki niezbÍdne do tworzenia
aplikacji wykorzystuj±cych moøliwo∂ci menadøera okien WindowMaker.
